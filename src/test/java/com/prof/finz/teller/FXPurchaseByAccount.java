...
public class FXPurchaseByAccount extends Utility{
	
	private WebDriver driver;
	
	public FXPurchaseByAccount(){
...
public static final Logger log = Logger.getLogger(BillPaymentByCash.class.getName());
	...
	public void UpdatingData() throws Exception {
		...
		for (Map<String, String> data : pram) {
			...
			if(data.get("Action").equalsIgnoreCase("FXPurchaseByAccount")) {
				
				driver.findElement(By.xpath("//div[contains(text(),'FX Purchase By Account')]")).click();
				log.info("Clicked on FX Purchase By Account");
